...
func New() Model {
	return Model{
		lines:        []string{},
		filtered:     []string{},
		status:       i18n.T("Connecting to Log Engine..."),
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		if m.pickerOpen {
			...
			case "enter":
				...
				m.filtered = []string{}
				m.scrollOffset = 0
				m.status = i18n.Tf("Switched to %s", m.sources[m.activeSource])
			}
			return m, nil
		}
		...
		case "p", "P":
			m.paused = !m.paused
		case "up", "k":
			if m.scrollOffset < len(m.filtered)-1 {
				m.scrollOffset++
			}
	...
	case sshlib.ConnectedMsg:
		...
		m.status = i18n.T("Connected. Press 's' to choose a log source.")
		return m, func() tea.Msg {
			srcs, err := m.engine.DiscoverSources()
			if err == nil && len(srcs) > 0 {
				return sourcesLoadedMsg(srcs)
	...
	case agent.Payload:
		if !m.paused && m.engine != nil && !m.pickerOpen {
			...
			return m, func() tea.Msg {
				...
				if err != nil {
					return logsLoadedMsg(i18n.T("Error fetching logs: ") + err.Error())
				}
...
func (m Model) View() string {
	...
	if m.pickerOpen {
		...
		b.WriteString(components.Title(i18n.T("LOG SOURCES")) + "\n\n")
		...
		b.WriteString(lipgloss.NewStyle().Foreground(dimColor).Render("\n" + i18n.T("Press Enter to select, ESC to cancel.")))
		return components.Card(b.String(), 90)
	}

	var status string
	if m.paused {
		status = lipgloss.NewStyle().Foreground(theme.Current.Warning).Bold(true).Render(i18n.T("[PAUSED (Auto-scroll disabled)]"))
	} else {
		status = lipgloss.NewStyle().Foreground(theme.Current.Success).Render(i18n.T("[LIVE STREAMING]"))
	}
	...
	if len(m.filtered) == 0 {
		logOutput.WriteString(lipgloss.NewStyle().Foreground(dimColor).Render(i18n.T("No logs found.")))
	}

	controls := lipgloss.NewStyle().Foreground(dimColor).Render("\n" + i18n.T("[s] Change Source  [P] Pause/Resume  [Up/Dn] Scroll  [/] Filter  [ESC] Clear"))

	headerTitle := i18n.Tf("LOG VIEWER — %s", m.sources[m.activeSource])
