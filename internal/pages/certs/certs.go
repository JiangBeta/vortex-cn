...
func New() Model {
	return Model{
		certs:   []certsengine.CertInfo{},
		loading: false,
		status:  i18n.T("Connecting to Certs Engine..."),
	}
}
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "r", "R":
			if len(m.certs) > 0 {
				...
				m.isRenewing = true
				m.renewOutput = i18n.Tf("Running certbot renew for %s...\n", cert.Name)
				m.status = i18n.T("Renewing...")
	...
	case sshlib.ConnectedMsg:
		...
		m.status = i18n.T("Connected. Scanning for SSL Certificates...")
		m.loading = true
		return m, func() tea.Msg {
	...
	case certsLoadedMsg:
		...
		if len(m.certs) > 0 && m.certs[0].DaysLeft < 14 {
			...
			m.status = i18n.Tf("⚠️ WARNING: %s expires in %d days!", m.certs[0].Name, m.certs[0].DaysLeft)
		}
	...
	case renewCompleteMsg:
		if msg.err != nil {
			m.renewOutput += "\n\n" + i18n.T("❌ Error:\n") + msg.err.Error() + "\n" + msg.output
			m.status = i18n.T("Renewal Failed")
		} else {
			m.renewOutput += "\n\n" + i18n.T("✅ Renewal Successful!\n") + msg.output
			m.status = i18n.T("Renewal Successful")
...
func (m Model) View() string {
	...
	if m.isRenewing {
		...
		b.WriteString(components.Title(i18n.T("CERTIFICATE RENEWAL")) + "\n\n")
		b.WriteString(lipgloss.NewStyle().Foreground(theme.Current.Text).Render(m.renewOutput) + "\n\n")
		b.WriteString(lipgloss.NewStyle().Foreground(dimColor).Render(i18n.T("Press ESC to return.")))
		return components.Card(b.String(), 90)
	}

	var b strings.Builder
	b.WriteString(components.Title(i18n.T("SSL CERTIFICATE MANAGER")) + "\n\n")

	if m.loading && len(m.certs) == 0 {
		b.WriteString(lipgloss.NewStyle().Foreground(dimColor).Render(i18n.T("Scanning certificates...\n")))
	} else if len(m.certs) == 0 {
		b.WriteString(lipgloss.NewStyle().Foreground(theme.Current.Warning).Render(i18n.T("No certificates found or certbot is not installed.\n")))
	} else {
		for i, c := range m.certs {
			...
			daysStr := daysStyle.Width(15).Render(i18n.Tf("%d days left", c.DaysLeft))
	...
	b.WriteString("\n" + lipgloss.NewStyle().Foreground(dimColor).Render(i18n.T("[R] Renew Certificate   [Up/Down] Navigate")))
