...
func New() Model {
	...
	bInput.Placeholder = i18n.T("Command to run (e.g. apt update, systemctl restart nginx)")
	bInput.Focus()
	...
	m := Model{
		servers:     cfg.Servers,
		cursor:      0,
		status:      i18n.T("Select server (Enter), Multi-select (Space), Bulk action (B)"),
	...
	for i := range m.inputs {
		...
		switch i {
		case 0:
			t.Placeholder = i18n.T("Server Name (e.g. Production Web)")
			t.Focus()
		case 1:
			t.Placeholder = i18n.T("Host / IP Address")
		case 2:
			t.Placeholder = i18n.T("Port (22)")
			t.SetValue("22")
		case 3:
			t.Placeholder = i18n.T("Username (root)")
			t.SetValue("root")
		case 4:
			t.Placeholder = i18n.T("Password OR Path to SSH Key (~/.ssh/id_rsa)")
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
		...
		if m.currentMode == modeForm {
			...
			case "tab", "shift+tab", "enter", "up", "down":
				...
				if s == "enter" && m.focusIndex == len(m.inputs) {
					...
					m.status = i18n.T("✅ Server saved successfully!")
					return m, nil
				}
	...
	case sshlib.ConnectedMsg:
		m.status = i18n.T("✅ Connected! Deploying Agent...")
		m.client = msg.Client
		return m, nil
	}
	return m, nil
...
func (m *Model) instantConnect() (tea.Model, tea.Cmd) {
	...
	m.status = i18n.Tf("Connecting to %s...", s.Name)

	return *m, func() tea.Msg {
		client, err := sshlib.Connect(s.Host, s.Port, s.User, s.Password, s.KeyPath)
		if err != nil {
			return fmt.Errorf("%s", i18n.Tf("❌ Connection failed: %v", err))
		}
...
func (m Model) View() string {
	...
	if m.currentMode == modeBulkAction {
		...
		b.WriteString(components.Title(i18n.T("FLEET-WIDE BULK ACTION")) + "\n\n")

		if m.bulkJobs == nil {
			// Input phase
			b.WriteString(m.bulkInput.View() + "\n\n")
			b.WriteString(lipgloss.NewStyle().Foreground(dimColor).Render(i18n.T("Press Enter to execute across all selected servers. ESC to cancel.")))
		...
		header := i18n.Tf("Summary: %d/%d completed. %d succeeded, %d failed.\n\n", totalDone, len(m.bulkJobs), successCount, failCount)
		...
		for _, j := range m.bulkJobs {
			...
			statusStr := ""
			if !j.Done {
				statusStr = lipgloss.NewStyle().Foreground(theme.Current.Warning).Render(i18n.T("⏳ Running..."))
			} else if j.Success {
				statusStr = lipgloss.NewStyle().Foreground(theme.Current.Success).Render(i18n.T("✓ Completed"))
			} else {
				statusStr = lipgloss.NewStyle().Foreground(theme.Current.Error).Render(i18n.T("✗ Failed"))
			}
	...
	if m.currentMode == modeForm {
		var b strings.Builder
		b.WriteString(components.Title(i18n.T("ADD NEW SERVER")) + "\n\n")
	...
	var items string
	items += components.Title(i18n.T("REGISTERED SERVERS")) + "\n\n"
	...
	items += fmt.Sprintf("\n%s %s\n", cursor, btnStyle.Render(i18n.T("[+] Add New Server")))
