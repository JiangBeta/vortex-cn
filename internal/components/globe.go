...
func (g *Globe) View(width, height int, payload agent.Payload, activeHost string, isFetching bool) string {
	...
	if g.IsEntering && g.EnterProgress < 2 {
		return lipgloss.Place(width, height, lipgloss.Center, lipgloss.Center, i18n.T("Initializing Mission Control Core..."))
	}
	...
	if trafficIntensity > 100 {
		trafficIntensity = 100
	}
	...
	lineChar := "─"
	if trafficIntensity > 5 {
		lineChar = "═"
	}
	if trafficIntensity > 50 {
		lineChar = "█"
	}
	...
	for i, srv := range g.Servers {
		...
		lineColor := dim
		if isActiveSrv {
			lineColor = theme.Current.Primary
		}
		if i == g.SelectedIdx {
			lineColor = theme.Current.Text
	...
	leftPanel := lipgloss.NewStyle().
		...
		Render(lipgloss.JoinVertical(lipgloss.Left,
			lipgloss.NewStyle().Bold(true).Foreground(text).Render(i18n.T("INFRASTRUCTURE TOPOLOGY")),
			"",
			summaryBox,
			"",
			strings.Join(nodes, "\n\n"),
	...
	if len(g.Servers) > 0 && g.Servers[g.SelectedIdx].Host == activeHost {
		...
		sysGroup := lipgloss.JoinVertical(lipgloss.Left,
			lipgloss.NewStyle().Foreground(accent).Bold(true).Render(i18n.T("SYSTEM")),
			lipgloss.NewStyle().Foreground(dim).Render("━━━━━━━━━━━━━━━━━━━━━━━━━━━━"),
			fmt.Sprintf("%-12s %s", i18n.T("Hostname:"), payload.Network.Hostname),
			fmt.Sprintf("%-12s %s", i18n.T("OS:"), payload.Stats.OS),
			fmt.Sprintf("%-12s %s", i18n.T("Kernel:"), payload.Stats.Kernel),
			fmt.Sprintf("%-12s %s", i18n.T("Uptime:"), payload.Stats.Uptime),
		)

		// Resources
		resGroup := lipgloss.JoinVertical(lipgloss.Left,
			lipgloss.NewStyle().Foreground(accent).Bold(true).Render(i18n.T("RESOURCES")),
			lipgloss.NewStyle().Foreground(dim).Render("━━━━━━━━━━━━━━━━━━━━━━━━━━━━"),
			fmt.Sprintf("%-12s %.1f%%", i18n.T("CPU:"), payload.Stats.CPUPercent),
			fmt.Sprintf("%-12s %.1f%%", i18n.T("Memory:"), payload.Stats.RAMPercent),
			fmt.Sprintf("%-12s %.1f%%", i18n.T("Disk:"), payload.Stats.DiskPercent),
		)

		// Network
		netGroup := lipgloss.JoinVertical(lipgloss.Left,
			lipgloss.NewStyle().Foreground(accent).Bold(true).Render(i18n.T("NETWORK")),
			lipgloss.NewStyle().Foreground(dim).Render("━━━━━━━━━━━━━━━━━━━━━━━━━━━━"),
			fmt.Sprintf("%-12s %s", i18n.T("Public IP:"), payload.Network.PublicIP),
			fmt.Sprintf("%-12s %d", i18n.T("Interfaces:"), len(payload.Network.Interfaces)),
			fmt.Sprintf("%-12s %.2f Mbps", i18n.T("Bandwidth:"), totalMbps),
		)

		// Services
		svcGroup := lipgloss.JoinVertical(lipgloss.Left,
			lipgloss.NewStyle().Foreground(accent).Bold(true).Render(i18n.T("SERVICES")),
			lipgloss.NewStyle().Foreground(dim).Render("━━━━━━━━━━━━━━━━━━━━━━━━━━━━"),
			fmt.Sprintf("%-12s %d", i18n.T("Docker:"), payload.Docker.Containers),
			fmt.Sprintf("%-12s %d", i18n.T("Processes:"), len(payload.Services)),
		)
	...
	rightPanel := lipgloss.NewStyle().
		...
		Width(45).
		Render(lipgloss.JoinVertical(lipgloss.Left,
			lipgloss.NewStyle().Foreground(accent).Bold(true).Render(i18n.T("SERVER TELEMETRY")),
			"",
			infoContent,
		))
	...
	header := lipgloss.NewStyle().Foreground(text).Bold(true).Background(lipgloss.Color("236")).Padding(0, 2).Render(" " + i18n.T("MISSION CONTROL CORE") + " ")
	footer := lipgloss.NewStyle().Foreground(dim).Render(i18n.T("Press [G] or [ESC] to close  |  [Up/Down] Navigate"))
